...
def save_mask_data(output_dir, tags_chinese, mask_list, box_list, label_list):
    ...
    normalized_data = []
    for mask in json_data["mask"]:
        if "box" in mask:
            normalized_box = [coord / 1024 for coord in mask["box"]]
            normalized_data.append({"label": mask["label"], "box": normalized_box})
    
    with open(os.path.join(output_dir, 'label_processed.json'), 'w') as f:
...
if __name__ == "__main__":
    ...
    img_cats = os.listdir(img_path)
    for cat in tqdm(img_cats):
        os.makedirs(os.path.join(output_dir, cat), exist_ok=True)
        img_files = os.listdir(os.path.join(img_path, cat))
        for img in tqdm(img_files):
            image_path = os.path.join(img_path, cat, img)
            os.makedirs(os.path.join(output_dir, cat, img.split(".")[0]), exist_ok=True)
            # load image
            image_pil, image = load_image(image_path)
            
            # visualize raw image
            image_pil.save(os.path.join(os.path.join(output_dir, cat, img.split(".")[0]), "raw_image.jpg"))
            ...
            plt.axis('off')
            plt.savefig(
                os.path.join(os.path.join(output_dir, cat, img.split(".")[0]), "automatic_label_output.jpg"), 
                bbox_inches="tight", dpi=300, pad_inches=0.0
            )

            save_mask_data(os.path.join(output_dir, cat, img.split(".")[0]), tags_chinese, masks, boxes_filt, pred_phrases)
